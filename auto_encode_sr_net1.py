...
class GenBlock(nn.Module):
    def __init__(self, in_ch, out_ch, act, mode='up'):
        ...
        if mode == 'up':
            self.skip_conv1 = DeConvBnAct(in_ch, out_ch, 2, 2, 0, 0, False, None)
            # self.skip_conv1 = nn.Sequential(
            #     # nn.Conv2d(in_ch, in_ch, 3, 1, 1, groups=in_ch),
            #     ConvBnAct(in_ch, out_ch*4, 1, 1, 0, False, None),
            #     blur_pool,
            #     nn.PixelShuffle(2),
            # )
        elif in_ch != out_ch:
        ...
        if mode == 'up':
            self.conv1 = DeConvBnAct(in_ch, int(out_ch), 2, 2, 0, 0, True, act)
            # self.conv1 = nn.Sequential(
            #     # nn.Conv2d(in_ch, in_ch, 3, 1, 1, groups=in_ch),
            #     ConvBnAct(in_ch, out_ch*2, 3, 1, 1, True, act),
            #     nn.PixelShuffle(2)
            # )
        else:
...
class GenNet(nn.Module):
    def __init__(self):
        ...
        self.color_conv1 = ConvBnAct(64, 256, 3, 1, 1, True, act)
        ...
        # 64x64
        # 48x48
        self.edge_conv1 = ConvBnAct(1, 32, 3, 1, 1, True, act)
        ...
        self.img_conv = ConvBnAct(32, 3, 1, 1, 0, False, nn.Tanh())
...
class EncBlock(nn.Module):
    def __init__(self, in_ch, out_ch, act, mode='down'):
        ...
        # 必须要加bn，不然会数值会爆炸
        if mode == 'down':
            self.conv_skip = nn.Sequential(ConvBnAct(in_ch, out_ch, 2, 2, 0, False, None),
                                           blur_pool)
        elif in_ch != out_ch:
        ...
        if mode == 'down':
            self.conv1 = nn.Sequential(ConvBnAct(in_ch, int(out_ch*1), 3, 2, 1, True, act),
                                       blur_pool)
        else:
            self.conv1 = ConvBnAct(in_ch, int(out_ch*1), 3, 1, 1, True, act)
        self.conv2 = ConvBnAct(int(out_ch*1), out_ch, 3, 1, 1, True, act)
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.edge_conv2 = ConvBnAct(32, 1, 3, 1, 1, False, nn.Tanh())
        ...
        self.color_conv2 = ConvBnAct(256, 64, 3, 1, 1, False, nn.Tanh())
